...
# ====================== GeoJSON转换器 ======================
class GeoJSONYOLOConverter(BaseYOLOConverter):
    """处理GeoJSON标注的YOLO转换器"""
    CLASS_MAPPING = {
...
# ====================== KVJSON转换器 ======================
class KVYOLOConverter(BaseYOLOConverter):
...
def main():
    ...
    parser.add_argument('--converter_type', choices=['geojson', 'kvjson'], required=True)

    args = parser.parse_args()

    # 选择转换器类型
    if args.converter_type == 'geojson':
        converter = GeoJSONYOLOConverter(
